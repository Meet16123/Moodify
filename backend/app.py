...
def callback():
    ...
    # Clear any cached tokens that might be revoked
    try:
        cache_path = sp_oauth.cache_handler.cache_path
        if os.path.exists(cache_path):
            os.remove(cache_path)
            print(f"Cleared cached token file: {cache_path}")
    except Exception as e:
        print(f"Could not clear cache (this is okay if cache doesn't exist): {e}")
    ...
    try:
        ...
        # If there's still an error, try clearing cache again and retry once
        try:
            cache_path = sp_oauth.cache_handler.cache_path
            if os.path.exists(cache_path):
                os.remove(cache_path)
...
def adjust_volume_api():
    ...
    try:
        ...
        if playback and playback.get('device'):
            ...
            if playback['device'].get('volume_percent') is None:
        ...
        # Try to set volume, but handle errors gracefully
        try:
            sp.volume(recommended_volume, device_id=device_id)
            print(f"[API] Set Spotify volume to: {recommended_volume} (device: {device_name})")
            return jsonify({
                "message": f"Spotify volume set to {recommended_volume}",
                "volume": recommended_volume,
                "device": device_name
            })
        except Exception as volume_error:
            error_str = str(volume_error)
            # Check if it's a volume control disallowed error
            if "403" in error_str or "VOLUME_CONTROL_DISALLOW" in error_str:
                print(f"[API] Volume control not supported on device: {device_name}")
                return jsonify({
                    "message": "Volume control not available on this device",
                    "volume": recommended_volume,
                    "device": device_name,
                    "warning": "Your device doesn't support remote volume control. Please adjust volume manually."
                }), 200  # Return 200 OK with warning instead of error
            else:
...
def set_spotify_volume_api():
    ...
    try:
        ...
        try:
            ...
            print(f"[API] Set Spotify volume to: {volume} (device: {device_name}) via slider")
            return jsonify({
...
if __name__ == '__main__':
    ...
    print("🚀 Starting Moodify Flask app...")
